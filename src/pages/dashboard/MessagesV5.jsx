...
import {
  Bell, ChevronDown, Search, Lock, Bot,
  AlertCircle, Pill, Activity, HeartPulse,
...
function SectionHeader({ icon, title }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}>
      <span style={{ color: '#1F3A5F', display: 'flex', alignItems: 'center' }}>{icon}</span>
...
export default function MessagesV5() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: PAGE_BG, fontFamily: FF, display: 'flex' }}>
      <SidebarV5 active="messages" collapsed={navCollapsed} onToggle={() => setNavCollapsed(o => !o)} />
      ...
      <div style={{ marginLeft: navCollapsed ? COLLAPSED_W : NAV_W, flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, transition: 'margin-left 0.22s ease' }}>
        ...
        {/* ── Top bar ── */}
        <div style={{ height: TOPBAR_H, background: '#ffffff', borderBottom: '1px solid #E8EDF2', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', flexShrink: 0 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <h1 style={{ fontSize: 17, fontWeight: 700, color: '#0F172A', margin: 0, fontFamily: FF, letterSpacing: '-0.01em', lineHeight: 1.2 }}>Messages</h1>
            <p style={{ fontSize: 12.5, color: '#94A3B8', margin: 0, fontFamily: FF, lineHeight: 1.3 }}>Team communication and patient messaging</p>
          </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: isActive ? 'linear-gradient(135deg, rgba(31,58,95,0.05) 0%, rgba(13,148,136,0.07) 100%)' : 'transparent',
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
                          <span style={{ fontSize: 13.5, fontWeight: 600, color: '#0F172A', fontFamily: FF, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 120 }}>{session.patient.name}</span>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
                            {session.unread > 0 && (
                              <span style={{ width: 18, height: 18, borderRadius: '50%', background: GRADIENT, color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: FF }}>{session.unread}</span>
                            )}
            ...
            <div style={{ padding: '18px 18px 14px', borderBottom: '1px solid #F1F5F9' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center' }}>
                  {p?.programs.map(prog => (
                    <span key={prog} style={{ fontSize: 11, fontWeight: 600, padding: '2px 9px', borderRadius: 999, background: 'linear-gradient(135deg, rgba(31,58,95,0.08) 0%, rgba(13,148,136,0.08) 100%)', color: '#1F3A5F', border: '1px solid rgba(31,58,95,0.12)', fontFamily: FF }}>{prog}</span>
                  ))}
            ...
            <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 22 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {p?.programs.map(prog => (
                    <div key={prog} style={{ flex: 1, padding: '10px 12px', borderRadius: 9, background: 'linear-gradient(135deg, rgba(31,58,95,0.06) 0%, rgba(13,148,136,0.06) 100%)', border: '1px solid rgba(13,148,136,0.18)', textAlign: 'center' }}>
                      <p style={{ fontSize: 14, fontWeight: 800, color: '#1F3A5F', margin: 0, fontFamily: FF }}>{prog}</p>
                      <p style={{ fontSize: 10.5, color: TEAL, margin: '2px 0 0', fontFamily: FF }}>Active</p>
                    </div>
                  ))}
                </div>
              </div>
