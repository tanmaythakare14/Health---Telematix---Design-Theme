...
import {
  ...
  Bell, LogOut, ChevronDown, Search, Lock, Bot,
  AlertCircle, Pill, Activity, HeartPulse, Weight,
  Users, UserCheck, ChevronRight, Clock,
} from 'lucide-react'
                  ...
                  return (
                    <div key={msg.id} style={{ display: 'flex', flexDirection: isPatient ? 'row-reverse' : 'row', alignItems: 'flex-start', gap: 10 }}>
                      ...
                      <div style={{ maxWidth: '68%', display: 'flex', flexDirection: 'column', gap: 4, alignItems: isPatient ? 'flex-end' : 'flex-start' }}>
                        ...
                        {/* Bubble */}
                        <div style={{ padding: '10px 14px', borderRadius: isPatient ? '14px 4px 14px 14px' : '4px 14px 14px 14px', background: style.bg, border: `1px solid ${style.border}`, fontSize: 13.5, color: style.text, lineHeight: 1.6, fontFamily: FF }}>
                          {msg.text}
                        </div>
                      </div>
                    </div>
